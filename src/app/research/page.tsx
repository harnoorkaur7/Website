    ...
    <div className="space-y-10 text-slate-700">
      ...
      <section className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <h3 className="text-2xl font-semibold text-slate-900">Selected work</h3>
          <span className="text-xs uppercase tracking-[0.35em] text-slate-500">
            2022—2024
          </span>
        </div>
        <div className="space-y-6 divide-y divide-slate-200 border border-slate-200">
          {studies.map((study) => (
            <article key={study.title} className="space-y-4 p-6">
      ...
      <section className="grid gap-8 border-t border-slate-200 pt-8 md:grid-cols-2">
        ...
        <ul className="space-y-3">
          {methods.map((method) => (
            <li key={method} className="rounded-lg border border-slate-200 p-4">
